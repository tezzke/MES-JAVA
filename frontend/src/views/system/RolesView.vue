...
async function save() { await systemApi.saveRole(editingId.value,form); dialog.value=false; ElMessage.success('岗位已保存'); await load(); }
async function remove(row: Role) { await ElMessageBox.confirm(`确认删除岗位 ${row.name}？`,'删除确认',{type:'warning'}); await systemApi.deleteRole(row.id); await load(); }
...
<template>
  <div class="page">
    <PageHeader :title="PAGE_COPY['/system/roles'].title" :subtitle="PAGE_COPY['/system/roles'].subtitle" />
    <div class="panel action-panel"><el-button v-if="auth.has('ROLE_WRITE')" type="primary" @click="open()">新增岗位</el-button><el-button @click="load">刷新</el-button></div>
    <div class="panel table-panel">
      <el-table :data="rows">
        <el-table-column prop="code" label="岗位编码" /><el-table-column prop="name" label="岗位名称" />
    ...
    <el-dialog v-model="dialog" :title="editingId?'编辑岗位':'新增岗位'" width="480">
      <el-form :model="form" label-width="90"><el-form-item label="岗位编码"><el-input v-model="form.code" maxlength="64" /></el-form-item><el-form-item label="岗位名称"><el-input v-model="form.name" maxlength="100" /></el-form-item></el-form>
