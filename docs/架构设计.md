...
| 厂房与设备外观拆到 plant.json,不与点表同文件 | 两份数据的责任人与修改时机不同:点表归电气调试、厂房模型归图纸与协议书。拆开后任一方改动都不会波及另一方,也避免高频改点表时误伤 3D 模型 |
...
│           └── resources/
│               ├── devices.json          #   ★ 设备点表配置(采集侧:IP / 点表 / 场景坐标)
│               ├── plant.json            #   ★ 厂房与设备外观模型(展示侧:轴网 / 墙体 / 分区 / 外观)
...
│       ├── three/                        #   3D 车间(纯渲染,无业务):FactoryScene 编排
│       │                                 #   + PlantBuilder 厂房壳体 + DeviceModelFactory 设备外观
...
flowchart TB
    ...
    subgraph 组件层
        ...
        C4[StatusTag]
        T[three/FactoryScene.ts<br/>场景编排,无业务]
        T1[three/PlantBuilder.ts<br/>厂房壳体]
        T2[three/DeviceModelFactory.ts<br/>设备参数化外观]
    end
...
3D 车间的几何全部由后端配置驱动,前端不写死任何尺寸。这样做的直接理由是:
厂房数据来自施工图、设备外观来自各设备的技术协议书,两者都会随现场复测反复修正,
若几何写在代码里,每次修正都要改前端并重新构建。

**三份数据、三个责任人:**

| 数据 | 来源 | 配置位置 | 谁改 |
|---|---|---|---|
| 厂房壳体(轴网 / 墙体 / 分区) | 施工图 DXF | `plant.json` → `Plant` | 土建/图纸 |
| 设备外观(机身尺寸 / 构件 / 配色) | 设备技术协议书 | `plant.json` → `DeviceModels` | 设备工程 |
| 设备定位与点表 | 现场电气调试 | `devices.json` | 电气调试 |

两个文件靠设备的 `Type` 字段弱关联:`devices.json` 只声明设备是什么型号,
长什么样在 `plant.json` 的 `DeviceModels` 里定义;查不到对应外观时前端退化为通用体块并打印告警。

**渲染分工:**

- `PlantBuilder` —— 地面、分区色块、轴网线、结构柱、外墙、隔墙、玻璃观察窗、轴号标牌。
  柱子用 `InstancedMesh` 合批,柱数随轴网增长也只有一次 draw call;相机初始视角按厂房尺寸自动取景,换厂房无需改代码。
- `DeviceModelFactory` —— 把 `ModelPart` 的构件描述翻译成几何,支持
  `box` / `cylinder` / `glass` / `screen` / `tower` / `rotor` / `frame` / `pipe` / `duct` 九种。
  另外画出图纸实测的地面工位框,用于核对设备之间是否留足了通道。
- `FactoryScene` —— 只做编排:组装光照、挂载厂房与设备、处理点击拾取与动画循环。
  实时状态由 `Factory3DView` 灌入,`tower` 构件的灯罩按设备状态着色,`rotor` 构件在设备运行时旋转。

**坐标体系:** 单位米,原点为建筑轴线 3-F 与 3-1 的交点。X 向东为正,Z 向南为正
(图纸的 +Y 北向对应场景 -Z)。由 CAD 毫米坐标换算:

```
X = (CAD_x - 21538.8) / 1000
Z = -(CAD_y - 20804.8) / 1000
```

`devices.json` 里每台设备都在注释中保留了 CAD 毫米原值,可直接回图核对。

**配置校验:** 跨两个文件的引用完整性靠人眼核对不现实,而写错的表现是"设备在 3D 里不见了/
变成灰盒子/穿到墙外",到现场才发现代价很高。因此 `PlantConfigConsistencyTest` 把这些约束固化成断言,
构建阶段即拦住:设备类型都有外观模型、点位模板都能解析、设备编码唯一、每套模板恰好一个状态点与一个累计点、
设备坐标落在外墙内且属于某个分区、构件不超出机身包络、墙段长度非零、外观模型都被引用且都标注了尺寸出处。
其中**柱距必须为 10.5 m** 这一条是坐标换算的锚点 —— 换算式或原点写错时它立刻不成立。
